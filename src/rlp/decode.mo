...
module {
  ...
  private func decodeLongString(input : Types.Uint8Array) : Result<Output, Text> {
    ...
    //   case (#err(val)) { return #err(val) };
    // };
    let length : Nat = bytestoNat(inputSlice);
    if (length <= 55) {
    ...
    //   case (#err(val)) { return #err(val) };
    // };
    let data = switch (Utils.safeSlice(input, 1 + Nat8.toNat(llength), 1 + Nat8.toNat(llength) + length)) {
      case (#ok(val)) { val };
      case (#err(val)) { return #err(val) };
    };
    let _remainder = switch (Utils.safeSlice(input, 1 + Nat8.toNat(llength) + length, input.size())) {
      case (#ok(val)) { val };
      case (#err(val)) { return #err(val) };
    };
  ...
  private func bytestoNat(vs : Types.Uint8Array) : Nat {
    var v : Nat = 0;
    for (vv in vs.vals()) {
      v := v * 256 + Nat8.toNat(vv);
    };
    v;
  };
  ...
  private func decodeLongList(input : Types.Uint8Array) : Result<Output, Text> {
    ...
    //   case (#err(val)) { return #err(val) };
    // };
    var innerRemainder = switch (Utils.safeSlice(input, Nat8.toNat(llength) + 1, totalLength)) {
      case (#ok(val)) { val };
      case (#err(val)) { return #err(val) };
    };
    let decoded = Buffer.Buffer<Types.Decoded>(1);
